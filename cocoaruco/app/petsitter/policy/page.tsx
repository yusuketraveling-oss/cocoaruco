...
export default function PolicyPage() {
  return (
    <main className="bg-white">
      ...
      {/* ─────────────── 全体目次 ─────────────── */}
      <section className="py-12 md:py-16 border-b border-sitter-main/10">
        <div className="max-w-3xl mx-auto px-5 md:px-8">
          <p className="text-xs tracking-widest text-sitter-accent font-bold mb-5 uppercase">
            目次
          </p>
          <ol className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2.5">
            {CHAPTERS.map((c) => (
              <li key={c.id}>
                <a
                  href={`#${c.id}`}
                  className="flex items-baseline gap-3 py-1 text-sm text-brand-navy hover:text-sitter-main transition-colors"
                >
                  <span className="text-[10px] tracking-widest text-sitter-accent font-bold shrink-0">
                    {c.number}
                  </span>
      ...
      {/* ─────────────── 各章 ─────────────── */}
      <article className="max-w-3xl mx-auto px-5 md:px-8 py-8 md:py-12">
      ...
      {/* ─────────────── 末尾の補足 ─────────────── */}
      <section className="py-16 md:py-20 bg-linen border-t border-sitter-main/10">
        <div className="max-w-3xl mx-auto px-5 md:px-8">
          <div className="rounded-2xl bg-white border border-sitter-main/10 p-6 md:p-8">
            <p className="text-xs tracking-widest text-sitter-accent font-bold mb-4 uppercase">
              規約について
            </p>
            <p className="text-sm md:text-base text-text-primary leading-loose mb-3">
              本ページは、cocoarucoペットシッターサービスをご利用いただく方に向けて、
              ご利用方法・料金・運用ルールなどを、わかりやすくまとめたものです。
            </p>
            <p className="text-sm md:text-base text-text-primary leading-loose mb-3">
              具体的な権利義務関係を定めた利用規約は、ご入会フローまたは
              Meet &amp; Greetの際にお渡しし、ご同意をいただいたうえで
              サービスを開始します。
            </p>
          ...
          <p className="mt-8 text-xs text-text-primary/50 leading-loose text-center">
